      ...
      <div className="bg-white w-full max-w-lg mx-auto rounded-t-3xl p-5 max-h-[90vh] overflow-y-auto"
        ...
        <div className="space-y-2.5">
          ...
          <div>
            ...
            {coins.length === 0 && (
              <p className="text-xs text-orange-500 mt-1">⚠️ 소속 조직에 코인이 없습니다.</p>
            )}
          </div>
        </div>
      ...
      <div className="bg-white border-b px-4 py-3 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400">주식 시장</p>
          </div>
          {/* 유저 프로필 버튼 */}
          <button onClick={() => setShowProfile(v => !v)}
            className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-2xl px-3 py-1.5">
            <div className="w-7 h-7 rounded-full bg-indigo-100 flex items-center justify-center text-sm font-black text-indigo-600">
              {user?.username?.[0]?.toUpperCase() || "?"}
            </div>
            <div className="text-left">
              <p className="text-xs font-bold text-gray-800 leading-tight">{user?.username}</p>
              <p className="text-[10px] text-gray-400 leading-tight">{user?.orgName || "조직 없음"}</p>
            </div>
          </button>
        </div>
      ...
      <div className="px-4 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-xs mt-1">우측 상단 프로필 버튼 → IPO 신청으로 첫 회사를 등록해보세요!</p>
          </div>
        ) : filtered.map(c => {
